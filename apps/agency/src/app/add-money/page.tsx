...
export default function AddMoneyPage() {
  ...
  // Generate Apply ID in format PWDYYYYMMDD{7 digit number}
  ...
  // Fetch agent's own pay link requests and settings when tab changes
  useEffect(() => {
      ...
      const fetchPayLinkData = async () => {
        ...
        try {
          const [requestsData, settingsData] = await Promise.all([
            transactionsApi.agentPayLinkRequests.getAll().catch(() => ({ payLinkRequests: [] })),
  ...
  // Fetch agent's own wallet flow when tab changes
  useEffect(() => {
    if (activeTab === 'wallet-flow') {
      const fetchWalletFlow = async () => {
        setLoadingWalletFlow(true)
        try {
          const data = await transactionsApi.agentWalletFlow.getAll()
  ...
  // Fetch users when distribute modal opens
  useEffect(() => {
    if (showDistributeModal && usersList.length === 0) {
  ...
  const handleSubmitDeposit = async () => {
    ...
    try {
      ...
      const result = await transactionsApi.agentDeposits.create({
        applyId: applyId,
      ...
      // Refresh agent deposits list
      const depositsData = await transactionsApi.agentDeposits.getAll()
  ...
  const handleSubmitPayLink = async () => {
    ...
    try {
      await transactionsApi.agentPayLinkRequests.create({
      ...
      // Refresh agent's own pay link requests
      const data = await transactionsApi.agentPayLinkRequests.getAll()
  ...
  // Submit distribute money handler
  const handleSubmitDistribute = async () => {
    if (!distributeForm.userId || !distributeForm.amount) {
    ...
    try {
      const result = await usersApi.distributeMoney(distributeForm.userId, {
        amount,
        remarks: distributeForm.remarks || undefined
      })

      // Update agent balance in store
      if (result.agentBalance !== undefined) {
        const userData = await authApi.me()
        if (userData.user) {
          setUser(userData.user)
        }
      ...
      // Refresh wallet flow if on that tab
      if (activeTab === 'wallet-flow') {
        const data = await transactionsApi.agentWalletFlow.getAll()
        setWalletFlows(data.flows || [])
      }
      ...
      // Reset form and close modal
      setDistributeForm({ userId: '', amount: '', remarks: '' })
      setShowDistributeModal(false)
      showToast(result.message || 'Money distributed successfully!', 'success')
    } catch (error: any) {
      console.error('Failed to distribute money:', error)
      showToast(error.message || 'Failed to distribute money', 'error')
    } finally {
  ...
  return (
    <DashboardLayout title="Add Money" subtitle="Manage your wallet deposits and transactions">
        ...
        <div className="p-5 border-b border-gray-100 bg-gradient-to-r from-white to-gray-50/50">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="relative flex-1 max-w-xs group">
              <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 transition-colors group-focus-within:text-[#7C3AED]" />
              <input
                type="text"
                placeholder="Search transactions..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-[#7C3AED]/20 focus:border-[#7C3AED] hover:border-gray-300"
              />
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              {/* Distribute Money Button */}
              <Button
                variant="outline"
                className="border-[#7C3AED] text-[#7C3AED] rounded-xl hover:bg-[#7C3AED]/10 transition-all duration-200"
              ...
              {activeTab === 'pay-link' ? (
                payLinkEnabled && (
                  <Button
                    className="bg-[#7C3AED] hover:bg-[#6D28D9] text-white rounded-xl shadow-lg shadow-purple-200/50 hover:shadow-purple-300/50 transition-all duration-300 hover:scale-[1.02]"
        ...
        <div className="px-5 pt-4 border-b border-gray-100 bg-white">
          <div className="flex items-center gap-2">
            {tabs.map((tab) => (
              <button
                ...
                className={`px-6 py-3 text-sm font-medium rounded-t-xl transition-all duration-300 relative overflow-hidden ${
                  activeTab === tab.id
                    ? 'bg-[#7C3AED] text-white shadow-lg shadow-purple-200/40'
            ...
            <div className="ml-auto flex items-center gap-2 text-sm animate-slideIn">
              ...
              <span className="px-3 py-1.5 bg-[#7C3AED] text-white rounded-full font-semibold shadow-md shadow-purple-200/50">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gradient-to-r from-gray-50 to-gray-100/50">
                  ...
                  <th className="text-left py-4 px-5 text-xs font-semibold text-[#7C3AED] uppercase tracking-wider">Charge Amount</th>
                  ...
                  filteredDeposits.map((item, index) => (
                    ...
                    >
                      <td className="py-3 px-4">
                        <span className="w-6 h-6 rounded-md bg-gray-100 flex items-center justify-center text-xs font-medium text-gray-600 group-hover:bg-[#7C3AED]/10 group-hover:text-[#7C3AED] transition-all duration-300">
                          {index + 1}
                        </span>
                      </td>
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-1.5">
                          <span className="text-xs text-gray-700 font-mono bg-gray-50 px-2 py-0.5 rounded whitespace-nowrap">
                            {item.applyId || `PWD${new Date(item.createdAt).toISOString().slice(0,10).replace(/-/g, '')}${item.id.slice(-7).toUpperCase()}`}
                          </span>
                          <button
                            onClick={() => copyToClipboard(item.applyId || `PWD${new Date(item.createdAt).toISOString().slice(0,10).replace(/-/g, '')}${item.id.slice(-7).toUpperCase()}`, `apply-${item.id}`)}
                      ...
                      <td className="py-3 px-4">
                        {item.paymentProof ? (
                          ...
                          >
                            <div className="relative w-8 h-8 rounded-md overflow-hidden border border-gray-200 group-hover/img:border-[#7C3AED] transition-all duration-300 shadow-sm">
                              <img
                    ...
                    {paginatedWalletFlows.map((flow, index) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-1.5">
                            <span className="text-xs text-gray-700 font-mono bg-gray-50 px-2 py-0.5 rounded whitespace-nowrap">
                              {flow.applyId || `PWD${new Date(flow.createdAt).toISOString().slice(0,10).replace(/-/g, '')}${flow.id.slice(-7).toUpperCase()}`}
                            </span>
                            <button
                              onClick={() => copyToClipboard(flow.applyId || `PWD${new Date(flow.createdAt).toISOString().slice(0,10).replace(/-/g, '')}${flow.id.slice(-7).toUpperCase()}`, `flow-${flow.id}`)}
                              className="flex-shrink-0 p-1 rounded hover:bg-gray-100 transition-colors"
                              title="Copy Apply ID"
                            >
                              {copiedId === `flow-${flow.id}` ? (
                                <Check className="w-3.5 h-3.5 text-green-500" />
                              ) : (
                                <Copy className="w-3.5 h-3.5 text-gray-400 hover:text-gray-600" />
                              )}
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                  <button
                    ...
                    className={`w-10 h-10 rounded-xl text-sm font-medium transition-all duration-300 ${
                      currentPage === page
                        ? 'bg-[#7C3AED] text-white shadow-lg shadow-purple-200/50 scale-110'
            ...
            <div className="p-5 border-t border-gray-100 flex items-center justify-between bg-gradient-to-r from-gray-50/50 to-white">
              ...
              <div className="flex items-center gap-3">
                ...
                <select
                  ...
                  className="px-3 py-2 border border-gray-200 rounded-lg text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#7C3AED]/20 focus:border-[#7C3AED] transition-all duration-200 bg-white"
                >
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#7C3AED]/20 focus:border-[#7C3AED] focus:bg-white"
              />
            </div>
          </div>
              ...
              <div className="bg-[#7C3AED]/10 border border-[#7C3AED]/30 rounded-xl p-4">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex-1">
                    <p className="text-xs text-[#7C3AED] font-medium mb-1">Payment Address / Details</p>
                  ...
                  <button
                    ...
                    className="flex-shrink-0 p-2 rounded-lg bg-[#7C3AED] text-white hover:bg-[#6D28D9] transition-colors"
          ...
          <div className="space-y-1.5">
            ...
            {filePreview ? (
              ...
              <div className="relative border-2 border-[#7C3AED] rounded-xl p-4 bg-purple-50/30">
                <div className="flex items-start gap-4">
                  {/* Image preview */}
                  <div className="relative w-24 h-24 rounded-lg overflow-hidden border-2 border-[#7C3AED]/30 shadow-sm">
                    <img
                      src={filePreview}
                      alt="Preview"
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <span className="inline-flex items-center gap-1 px-2 py-1 bg-[#7C3AED]/20 text-[#7C3AED] text-xs font-medium rounded-full">
                        <Check className="w-3 h-3" />
                        Uploaded
                      </span>
                    </div>
                ...
                <button
                  ...
                  className="mt-3 w-full py-2 text-sm text-[#7C3AED] font-medium border border-[#7C3AED]/30 rounded-lg hover:bg-purple-50 transition-colors"
                >
                  Change Image
                </button>
              </div>
              ...
              <div
                ...
                className="border-2 border-dashed border-[#7C3AED] rounded-xl p-8 text-center bg-purple-50/30 hover:bg-purple-50/50 transition-all duration-300 cursor-pointer group"
              >
          ...
          <div className="flex gap-3 pt-4">
            ...
            <Button
              className="flex-1 bg-[#7C3AED] hover:bg-[#6D28D9] rounded-xl py-3"
      ...
      >
        <p className="text-sm text-gray-500 -mt-2 mb-6">
          Transfer money from your wallet to one of your users.
        </p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Left Column - User Selection */}
          <div className="space-y-4">
            {/* Agent's Balance Display */}
            <div className="p-4 bg-gradient-to-r from-[#7C3AED] to-[#9333EA] rounded-xl text-white">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center">
                  <Wallet className="w-5 h-5" />
                </div>
                <div>
                  <p className="text-white/80 text-xs">Your Available Balance</p>
                  <p className="text-2xl font-bold">${Number(user?.walletBalance || 0).toFixed(2)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative" ref={userDropdownRef}>
                {/* Search Input */}
                <div
                  className={`flex items-center gap-2 px-4 py-3 bg-gray-50 border rounded-xl cursor-pointer transition-all duration-200 ${
                    showUserDropdown
                        ...
                        {usersList
                          ...
                          .map((userItem) => (
                            <div
                              ...
                              onClick={() => {
                                setDistributeForm({...distributeForm, userId: userItem.id})
                                setUserSearchQuery(userItem.username || userItem.email)
                                setShowUserDropdown(false)
                              }}
                            >
            ...
            {distributeForm.userId && (() => {
              ...
              return (
                <div className="bg-[#7C3AED]/5 border border-[#7C3AED]/20 rounded-xl p-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-full bg-[#7C3AED] flex items-center justify-center text-white font-semibold text-lg">
                  ...
                  <div className="mt-3 pt-3 border-t border-[#7C3AED]/20 flex items-center justify-between">
                    <span className="text-xs text-gray-500">Current Balance</span>
                    <span className="text-lg font-bold text-[#7C3AED]">${Number(selectedUser.walletBalance || 0).toFixed(2)}</span>
                  </div>
                </div>
              )
            })()}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#7C3AED]/20 focus:border-[#7C3AED] focus:bg-white"
                />
              </div>
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#7C3AED]/20 focus:border-[#7C3AED] focus:bg-white resize-none"
              />
            </div>

            {/* Transfer Summary */}
            {distributeForm.userId && distributeForm.amount && (
              <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                <h4 className="text-xs font-semibold text-gray-500 uppercase mb-3">Transfer Summary</h4>
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-500">Amount</span>
                    <span className="font-semibold text-gray-800">${Number(distributeForm.amount || 0).toFixed(2)}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-500">Your Balance After</span>
                    <span className={`font-semibold ${
                      Number(user?.walletBalance || 0) - Number(distributeForm.amount || 0) >= 0
                        ? 'text-green-600'
                        : 'text-red-500'
                    }`}>
                      ${(Number(user?.walletBalance || 0) - Number(distributeForm.amount || 0)).toFixed(2)}
                    </span>
                  </div>
                  {(() => {
                    const selectedUser = usersList.find(u => u.id === distributeForm.userId)
            ...
            <div className="p-3 bg-amber-50 rounded-xl border border-amber-100">
              <p className="text-xs text-amber-700">
                <span className="font-semibold">Note:</span> This will deduct the amount from your wallet and add it to the selected user's wallet immediately.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-3 pt-6 mt-6 border-t border-gray-100">
          ...
          <Button
            ...
            disabled={
              ...
              Number(distributeForm.amount) > Number(user?.walletBalance || 0)
            }
          >
            {submittingDistribute ? (
              <>
              ...
              <>
                <Send className="w-4 h-4 mr-2" />
                Distribute ${distributeForm.amount ? Number(distributeForm.amount).toFixed(2) : '0.00'}
              </>
            )}
          </Button>
        </div>
