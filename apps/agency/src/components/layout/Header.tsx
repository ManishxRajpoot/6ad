    ...
    <header className="bg-white px-3 lg:px-6 py-2.5 border-b border-slate-100 relative z-50">
      <div className="flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center flex-1 max-w-md mx-8">
          <div className="relative w-full">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-600 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#7C3AED]/20 focus:border-[#7C3AED] transition-all"
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 lg:gap-2">
          ...
          {/* Notification Bell */}
          <button className="relative p-2.5 hover:bg-[#7C3AED]/5 rounded-xl transition-all group">
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className="group"
            >
              <div className="flex items-center gap-2.5 pl-1 pr-2 py-1 rounded-xl hover:bg-[#7C3AED]/5 transition-all duration-300">
                <div className="relative">
                  {user?.profileImage ? (
                    <img
                      src={user.profileImage}
                      alt={user.username || 'Profile'}
                      className="w-9 h-9 rounded-xl object-cover shadow-sm ring-2 ring-white group-hover:ring-[#7C3AED]/20 transition-all duration-300"
                    />
                  ) : (
                    <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#7C3AED] to-[#9333EA] flex items-center justify-center shadow-sm ring-2 ring-white group-hover:ring-[#7C3AED]/20 transition-all duration-300">
                      <span className="text-white font-bold text-sm">
                        {user?.username?.charAt(0).toUpperCase() || 'P'}
                      </span>
                    </div>
                  )}
                  <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-[#7C3AED] rounded-full border-2 border-white" />
                </div>
                <div className="hidden sm:block text-left">
                  <p className="text-xs font-semibold text-slate-700 group-hover:text-[#7C3AED] transition-colors leading-none">
                    {user?.username || 'Partner'}
                  </p>
            ...
            >
              {/* User Info */}
              <div className="px-4 py-3 border-b border-slate-100 bg-gradient-to-r from-[#7C3AED]/5 to-[#9333EA]/5">
                <p className="text-sm font-semibold text-slate-800">{user?.realName || user?.username || 'Partner'}</p>
              ...
              <div className="py-1">
                <Link
                  ...
                  className="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-600 hover:bg-[#7C3AED]/5 hover:text-[#7C3AED] transition-all duration-200 group"
                >
                  <div className="w-8 h-8 rounded-lg bg-slate-100 group-hover:bg-[#7C3AED]/10 flex items-center justify-center transition-colors">
                    <UserCircle className="w-4 h-4 text-slate-500 group-hover:text-[#7C3AED] transition-colors" />
                  </div>
                ...
                >
                  <div className="w-8 h-8 rounded-lg bg-slate-100 group-hover:bg-[#7C3AED]/10 flex items-center justify-center transition-colors">
                    <ShieldCheck className="w-4 h-4 text-slate-500 group-hover:text-[#7C3AED] transition-colors" />
                  </div>
