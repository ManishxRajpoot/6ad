...
// POST /users/:id/reset-2fa - Admin resets 2FA for a user
users.post('/:id/reset-2fa', requireAdmin, async (c) => {
  try {
    const { id } = c.req.param()
    ...
    return c.json({
      message: `2FA has been reset for ${user.username}. They will be prompted to set up 2FA on next login.`
    })
  } catch (error) {
