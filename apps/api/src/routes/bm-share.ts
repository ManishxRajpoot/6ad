...
async function processBMShareInBackground(requestId: string, adAccountId: string, bmId: string, platform: string) {
  ...
  if (platform === 'FACEBOOK') {
    ...
    // ===== STEP 2: If Cheetah didn't handle it, leave PENDING for server BM Share Queue =====
    // The BM Share Queue polls for PENDING shares every 20s and processes them
    // via Facebook Graph API using cached tokens from the Token Pool.
    if (!cheetahHandled) {
      status = 'PENDING'
      adminRemarks = 'Cheetah failed — pending manual handling by admin.'
      shareMethod = 'MANUAL'
...
bmShare.get('/admin', requireAdmin, async (c) => {
  try {
    const { platform, status, page = '1', limit = '100', search } = c.req.query()
