  ...
  try {
    ...
    if (isCryptoPayment && transactionId) {
      ...
      const user = await prisma.user.findUnique({
        where: { id: userId },
        include: { agent: { select: { email: true, brandLogo: true, username: true, emailSenderNameApproved: true, smtpEnabled: true, smtpHost: true, smtpPort: true, smtpUsername: true, smtpPassword: true, smtpEncryption: true, smtpFromEmail: true, customDomains: { where: { status: 'APPROVED' }, select: { brandLogo: true, brandName: true }, take: 1 } } } }
      })

      if (user) {
        const approvedDomain = user.agent?.customDomains?.[0]
        const agentLogo = approvedDomain?.brandLogo || user.agent?.brandLogo || null
        const agentBrandName = approvedDomain?.brandName || user.agent?.username || null
...
transactions.post('/deposits/:id/approve', requireAdmin, async (c) => {
  try {
          ...
          include: {
            agent: {
              select: {
                ...
                smtpEncryption: true,
                smtpFromEmail: true,
                customDomains: { where: { status: 'APPROVED' }, select: { brandLogo: true, brandName: true }, take: 1 }
              }
            }
          }
        }
    ...
    if (isAgent) {
      ...
      const agentBrandNameApprove = approvedDomain?.brandName || deposit.user.agent?.username || null
      const userEmailTemplate = getWalletDepositApprovedTemplate({
        username: deposit.user.username,
        applyId: deposit.applyId,
        amount: Number(deposit.amount),
...
transactions.post('/deposits/:id/reject', requireAdmin, async (c) => {
  try {
    ...
    const deposit = await prisma.deposit.findUnique({
      where: { id },
      include: { user: { include: { agent: { select: { brandLogo: true, username: true, emailSenderNameApproved: true, smtpEnabled: true, smtpHost: true, smtpPort: true, smtpUsername: true, smtpPassword: true, smtpEncryption: true, smtpFromEmail: true, customDomains: { where: { status: 'APPROVED' }, select: { brandLogo: true, brandName: true }, take: 1 } } } } } }
    })
    ...
    const agentBrandNameReject = approvedDomainReject?.brandName || deposit.user.agent?.username || null
...
transactions.post('/agent-deposits', requireAgent, async (c) => {
  try {
    ...
    const agent = await prisma.user.findUnique({
      where: { id: agentId },
      select: { id: true, username: true, email: true, brandLogo: true, emailSenderNameApproved: true, smtpEnabled: true, smtpHost: true, smtpPort: true, smtpUsername: true, smtpPassword: true, smtpEncryption: true, smtpFromEmail: true, customDomains: { where: { status: 'APPROVED' }, select: { brandLogo: true, brandName: true }, take: 1 } }
    })
    ...
    if (isCryptoPayment && transactionId) {
      ...
      if (agent) {
        ...
        const agentBrandName = approvedDomain?.brandName || agent.username
