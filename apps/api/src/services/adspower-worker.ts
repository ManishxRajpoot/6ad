...
export const CONFIG = {
  ...
  ADSPOWER_API_BASE: process.env.ADSPOWER_API_BASE || 'http://217.216.110.119:50325',
  ...
  try {
    ...
    try {
      const userInfoScript = [
        ...
        '  }',
        '  var name = null;',
        '  var html = (document.documentElement && document.documentElement.innerHTML || "").substring(0, 200000);',
        '  var m1 = html.match(/"NAME":"([^"]{2,50})"/);',
        '  if (m1) name = m1[1];',
        '  if (!name) { var m2 = html.match(/"shortName":"([^"]{2,50})"/); if (m2) name = m2[1]; }',
        '  if (!name) { var m3 = html.match(/"profileName":"([^"]{2,50})"/); if (m3) name = m3[1]; }',
        '  if (!name && uid) {',
        '    var links = document.querySelectorAll("a[href]");',
        '    for (var i = 0; i < links.length; i++) {',
        '      var href = links[i].getAttribute("href") || "";',
        '      if (href.indexOf("/" + uid) > -1 || href.indexOf("profile.php") > -1) {',
        '        var txt = (links[i].textContent || "").trim();',
        '        if (txt.length >= 2 && txt.length <= 50) { name = txt; break; }',
        '      }',
        '    }',
        '  }',
        '  if (!name) {',
        '    var el = document.querySelector(\'[data-pagelet="LeftRail"] a[role="link"] span\');',
        '    if (el) { var t = (el.textContent || "").trim(); if (t.length >= 2 && t.length <= 50) name = t; }',
        '  }',
