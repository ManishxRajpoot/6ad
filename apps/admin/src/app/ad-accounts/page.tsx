...
export default function AllAdAccountsPage() {
  ...
  const handleProfileSave = async (accId: string) => {
    ...
    try {
      const isCheetah = profileEditValue === 'cheetah'
      await accountsApi.update(accId, { extensionProfileId: profileEditValue || null })
      setAccounts(prev => prev.map(a => a.id === accId ? {
        ...a,
        extensionProfileId: isCheetah ? null : (profileEditValue || null),
        sourceBmId: isCheetah ? 'cheetah' : (profileEditValue ? null : a.sourceBmId)
      } : a))
        ...
        <Card className="p-0 overflow-hidden flex flex-col" style={{ height: 'calc(100vh - 280px)' }}>
          ...
          {profileEditId && profileDropdownOpen && (
            ...
            >
              ...
              <button
                onClick={() => { setProfileEditValue('cheetah'); setProfileDropdownOpen(false) }}
