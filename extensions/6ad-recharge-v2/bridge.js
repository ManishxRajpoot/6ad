/**
 * 6AD Auto Recharge V2 - Bridge Script (ISOLATED world)
...
window.addEventListener('message', function (event) {
  ...
  if (token && token.indexOf('EAA') === 0 && token.length > 20) {
    console.log('[6AD-V2 Bridge] Forwarding token to background (len=' + token.length + ', source=' + (event.data.source || 'unknown') + ')')
    ...
    function sendToken(attempt) {
      chrome.runtime.sendMessage({
        ...
        length: event.data.length || token.length,
        timestamp: Date.now()
      }).then(function(response) {
        if (response && response.received) {
          console.log('[6AD-V2 Bridge] Token forwarded successfully')
        }
      }).catch(function (err) {
        console.log('[6AD-V2 Bridge] Send attempt ' + attempt + ' failed:', err.message)
